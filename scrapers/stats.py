...
async def get_vlr_client() -> httpx.AsyncClient:
    ...
    # bootstrap PHPSESSID
    await client.get(f"{VLR_BASE_URL}/stats")
...
async def vlr_stats(
    ...
    resp = await client.get("/stats", params=params)
    ...
    for item in html.css("tbody tr"):
        ...
        agents = [
            img.attributes["src"].split("/")[-1].split(".")[0]
            for img in item.css("td.mod-agents img")
        ]
