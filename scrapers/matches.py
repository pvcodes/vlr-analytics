...
def vlr_upcoming_matches_extended(
    num_pages=1,
    from_page=None,
    to_page=None,
    max_retries=3,
    ...
    print(
        f"Starting to scrape pages {start_page}-{end_page} ({total_pages} pages) with {request_delay}s delay between requests..."
    )
    ...
    for page in range(start_page, end_page + 1):
        ...
        while not page_success and retry_count < max_retries:
            try:
                ...
                print(
                    f"Scraping page {page} ({current_page_num}/{total_pages}) (attempt {retry_count + 1}/{max_retries})"
                )
                ...
                for item in items:
                    try:
                        ...
                        eta = (
                            item.css_first(".ml-status").text().strip()
                            if item.css_first(".ml-status")
                            else ""
                        )
                        ...
                        for team_div in team_divs:
                            team_name_elem = team_div.css_first(
                            ...
                            if flag_elem:
                                ...
                                if flag_class:
                                    flag = flag_class.replace("flag ", "").replace(
                                        " mod-", "_"
                                    )
                                    flags.append(flag)
                                else:
                        ...
                        if tourney_elem:
                            tourney_lines = [
                                line.strip()
                                for line in tourney_elem.text().split("\n")
                                if line.strip()
                            ]
                        ...
                        print(
                            f"Warning: Failed to parse match item on page {page}: {str(e)}"
                        )
                        continue
                ...
                retry_count += 1
                print(
                    f"Timeout error on page {page}, attempt {retry_count}/{max_retries}"
                )
                if retry_count < max_retries:
                ...
                retry_count += 1
                print(
                    f"Connection error on page {page}, attempt {retry_count}/{max_retries}"
                )
                if retry_count < max_retries:
                    backoff_time = request_delay * (2**retry_count)
                    print(f"Retrying page {page} in {backoff_time:.1f} seconds...")
            ...
            try:
                ...
                for item in items:
                    try:
                        ...
                        tourney_icon_url = (
                            f"https:{item.css_first('img').attributes['src']}"
                        )
