...
def vlr_events(upcoming=True, completed=True, page=1):
    ...
    def parse_events(container):
        ...
        for event_item in container.css("a.event-item"):
            ...
            event_status = extract_text_content(
                event_item.css_first(".event-item-desc-item-status")
            )

            # Extract complex parsed data
            prize = extract_prize_value(
                event_item.css_first(".event-item-desc-item.mod-prize")
            )
            ...
            thumb = normalize_image_url(
                img_elem.attributes.get("src", "") if img_elem else ""
            )
